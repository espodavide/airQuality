...
class TestSpatialJoinNaturalEarth:
    def setup_method(self):
        ...
        self.citiescoordinate = read_file(cities_path)
    ...
    def test_sjoin_inner(self):
        ...
        cities_with_country = sjoin(
            self.citiescoordinate, countries, how="inner", predicate="intersects"
        )
...
class TestNearest:
    ...
    def test_sjoin_nearest_inner(self):
        ...
        citiescoordinate = read_file(geopandas.datasets.get_path("naturalearth_cities"))
        countries = countries[["geometry", "name"]].rename(columns={"name": "country"})

        # default: inner and left give the same result
        result1 = sjoin_nearest(citiescoordinate, countries, distance_col="dist")
        assert result1.shape[0] == citiescoordinate.shape[0]
        result2 = sjoin_nearest(citiescoordinate, countries, distance_col="dist", how="inner")
        assert_geodataframe_equal(result2, result1)
        result3 = sjoin_nearest(citiescoordinate, countries, distance_col="dist", how="left")
        ...
        result4 = sjoin_nearest(citiescoordinate, countries, distance_col="dist", max_distance=1)
        assert_geodataframe_equal(
            result4, result1[result1["dist"] < 1], check_like=True
        )
        result5 = sjoin_nearest(
            citiescoordinate, countries, distance_col="dist", max_distance=1, how="left"
        )
        assert result5.shape[0] == citiescoordinate.shape[0]
